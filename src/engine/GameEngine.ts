...
// =================================================================================
// Helper Functions
// These are pure functions that were previously in gameSlice.ts or are newly created.
// =================================================================================
...
export class GameEngine {
  public handleEvent(state: GameState, event: GameEvent): EngineResult {
    switch (event.type) {
      case 'keydown':
        return this.handleKeydown(state, event.key);
      case 'chatresponse':
        return this.handleChatResponse(state, event);
      case 'timerelapsed':
        return this.handleTimerElapsed(state);
    }
  }
  ...
  private handleTimerElapsed(state: GameState): EngineResult {
    if (
      state.chatWindow &&
  ...
  private handleKeydown(state: GameState, key: string): EngineResult {
    if (state.splashText) {
      return key === 'Escape'
    ...
    if (state.chatWindow) {
      return this.handleChatKeydown(state, key);
    }

    return this.handleMovementKeydown(state, key);
  }

  private handleChatKeydown(state: GameState, key: string): EngineResult {
    const { chatWindow } = state;
    if (!chatWindow) return { state, actions: [] };

    switch (chatWindow.turnState.type) {
      case 'user_turn':
        return this.handleUserTurnKeydown(state, key);
      case 'confirming_action':
        return this.handleConfirmActionKeydown(state, key);
      case 'waiting_for_ai':
  ...
  private handleUserTurnKeydown(state: GameState, key: string): EngineResult {
    const { chatWindow } = state;
  ...
  private handleConfirmActionKeydown(
    state: GameState,
    key: string
  ): EngineResult {
    const { chatWindow } = state;
  ...
  private handleMovementKeydown(state: GameState, key: string): EngineResult {
    const direction = {
      ArrowUp: 'north',
      ArrowDown: 'south',
      ArrowLeft: 'west',
    ...
    if (direction) {
      const newState = this.handleMovement(state, direction as any);
      return { state: newState, actions: [] };
    }
    return { state, actions: [] };
  }
