...
export function TileRenderer({ tileset, tileArray, width, height }: TileRendererProps) {
  ...
  const renderFullTileArray = useCallback((ctx: CanvasRenderingContext2D, tiles: TileArray) => {
    // Clear the canvas
    ctx.fillStyle = '#000'
  ...
  const areTileLayersEqual = useCallback((indices1: number[], indices2: number[]): boolean => {
  ...
  const renderTileArrayDiff = useCallback((ctx: CanvasRenderingContext2D, newTiles: TileArray, currentTiles: TileArray) => {
    ...
    for (let y = 0; y < newTiles.tiles.length; y++) {
      for (let x = 0; x < newTiles.tiles[y].length; x++) {
        if (!areTileLayersEqual(newTiles.tiles[y][x], currentTiles.tiles[y][x])) {
  ...
  useEffect(() => {
    ...
    if (tileArray.tiles.length !== currentTileArray.tiles.length || 
